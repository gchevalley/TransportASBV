			...
			if (count($result) > 0) {
				...
				foreach ($result as $facture_individuelle ) {
					...
					$data_reboursement_transports_csv[] = array(
						...
						"KM",
						arrondi($remboursement_csv_total_km * $tmp_filiale->get_standard_prix_km() * ($tmp_filiale->get_standard_taux_compenation() / 100.0), 0.1),
						"<iban=" . $tmp_transporteur->get_iban() . "><communication1=" .  'Remboursement : ' . substr(mb_convert_encoding(ucfirst($mois_txt), 'ISO-8859-1', 'UTF-8'), 0, 3) . ' ' . $data_to_display['facturation_year']['value'] .  ">",
						date( 'Y-m-d h:i:s a', time() ),
