...
switch ($action) {
	...
	case "edit":
		if ( count($variables_to_clean[0]) == 0 ) { // GET + nothing in input
			if (isset($_GET['id'])) {
				echo Beneficiaire::form('edit', new Beneficiaire($_GET['id']));
			}
	 	} else if ($variables_to_clean[1] > 0) {
